...
import { mimeFromStorageKey } from '@/lib/image-utils-server';
...
/**
 * Get verification files for a request
 */
export async function getVerificationFiles(requestId: number): Promise<
  {
    id: number;
    storageKey: string;
    mimeType?: string | null;
...
> {
  const files = await prisma.verificationFile.findMany({
    where: { verificationId: requestId },
    select: {
      id: true,
      storageKey: true,
      mimeType: true,
    ...
    async () => {
      ...
      const file = await prisma.verificationFile.create({
        data: {
          ...
          role: fileData.role || Enum.VerificationFileRole.DOCUMENT,
        },
        select: { id: true },
      });
...
/**
 * Get verification requests for moderation (admin/moderator use)
 */
export async function getVerificationRequestsForModeration(params: {
  status?: VerificationStatus;
  cityId?: number;
  limit?: number;
  offset?: number;
...
> {
  const { status, cityId, limit = 20, offset = 0 } = params;
...
/**
 * Revoke user verification status (admin action)
 */
export async function revokeUserVerification(
  userId: string,
  revokerUserId: string,
  reason?: string,
  revokerRole: AuditActorRole = 'ADMIN'
): Promise<void> {
  const auditContext = await getEnhancedAuditContext();

  await auditServerAction(
    'VERIFICATION_REVOKE' as AuditAction,
    ...
    parseInt(userId), // User ID as entity
    `Verification status revoked: ${reason || 'No reason provided'}`
  );
}
